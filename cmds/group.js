...
kord({
  ...
  const ownerNumber = "2347019135989";
  
  if (msg === "codex owner") {
    const ownerName = "CODEX AI FOUNDER";
  ...
  const triggerPattern = /codex.*(developer|creator|tell me more|about)/;
  if (triggerPattern.test(msg)) {
    const ownerBio = "CODEX is the developer, official founder, and architect behind the Codex AI system. Dedicated to building innovative automated solutions with technology innovations. Inspired by the goal to ensure everyone has the most powerful WhatsApp bot ever created—your WhatsApp assistant, which assists you with many automation features and designs.";
...
If you have anything to say, you can contact my owner through this contact:
https://wa.me/${ownerNumber}

_Generated by Codex AI_`;
...
kord({
  ...
  if (msg === "codex server stats") {
    if (m.sender !== myNumber) return await m.react("🚫");

    const getUsage = () => {
      const current = Math.floor(Math.random() * 15); 
      const percent = Math.round((current / 60) * 100);
      return { current, percent };
    };

    const s1 = getUsage(); const s2 = getUsage(); const s3 = getUsage();
    const s4 = getUsage(); const s5 = getUsage(); const s6 = getUsage();
    const totalSessions = s1.current + s2.current + s3.current + s4.current + s5.current + s6.current;

    const stats = `*CODEX SERVER STATS*

*Server-1*
🖥️ Server-1
📊 🟢 ONLINE | Usage: ${s1.current}/60 (${s1.percent}%)

*Server-2*
🖥️ Server-2
📊 🟢 ONLINE | Usage: ${s2.current}/60 (${s2.percent}%)

*Server-3*
🖥️ Server-3
📊 🟢 ONLINE | Usage: ${s3.current}/60 (${s3.percent}%)

*Server-4*
🖥️ Server-4
📊 🟢 ONLINE | Usage: ${s4.current}/60 (${s4.percent}%)

*Server-5*
🖥️ Server-5
📊 🟢 ONLINE | Usage: ${s5.current}/60 (${s5.percent}%)

*Server-6*
🖥️ Server-6
📊 🟢 ONLINE | Usage: ${s6.current}/60 (${s6.percent}%)
...
kord({
  ...
  const bcRegex = /^codex\s+(?:after\s+(\d+)([smh])\s+)?broadcast\s+(.*)/i;
  const match = msg.match(bcRegex);
  ...
  if (match) {
    ...
    const broadcastMsg = match[3].trim();

    if (timeValue && timeUnit) {
      const units = { 's': 1000, 'm': 60000, 'h': 3600000 };
      const delayMs = parseInt(timeValue) * units[timeUnit.toLowerCase()];
      
      await m.reply(`Ok sir. Immediately it's ${timeValue}${timeUnit}, I will deliver your announcement to all groups.`);
    ...
    try {
      ...
      for (const group of groups) {
        try {
          await m.client.sendMessage(group.id, {
...
╚════════════════❒`;
      ...
      return m.reply(`❌ Broadcast Error: ${err.message}`);
    }
  }
});
