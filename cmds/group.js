...
kord({
  ...
  const msg = m.text ? m.text.toLowerCase() : "";
  
  const triggerPattern = /codex.*(developer|owner|creator)/;

  if (triggerPattern.test(msg)) {
    const ownerNumber = "2347019135989";
    const ownerBio = " CODEX IS  the the developer and also the official founder and architect behind the Codex AI system, dedicated to building innovative automated solutions, With technology innovations. With CODEX ai inspiration to make sure that everyone in the world has the most powerful ever created Whatsapp bot. also known as yout Whatsapp assistant, which assists you with with many automation features and designs.";
...
kord({
  ...
  const msg = m.text ? m.text.toLowerCase().trim() : "";

  if (msg === "codex owner") {
    const ownerNumber = "2347019135989";
    const ownerName = "CODEX"; 
