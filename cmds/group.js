...
kord({
  ...
  if (match) {
    if (m.sender !== allowedNumber) return await m.react("🚫");

    const timeValue = match[1]; 
    const timeUnit = match[2];  
    const broadcastMsg = match[3] ? match[3].trim() : "";

    if (!broadcastMsg) return m.reply("Please provide a message to broadcast!");

    if (timeValue && timeUnit) {
      const units = { s: 1000, m: 60000, h: 3600000 };
      const delayMs = parseInt(timeValue) * units[timeUnit];
      await m.reply(`🕒 *Timer set!* I will broadcast in ${timeValue}${timeUnit}.`);
